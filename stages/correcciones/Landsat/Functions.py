...
def array_to_raster(inTiff,array,outFile,dataType=gdal.GDT_Float32):
    ...
    """
    inDataset = gdal.Open(inTiff)
    transform = inDataset.GetGeoTransform()

    geotiff = gdal.GetDriverByName('GTiff')
    output = geotiff.CreateCopy(outFile, inDataset, 0)
    output.GetRasterBand(1).WriteArray(array)
    output.SetGeoTransform(transform)

    wkt = inDataset.GetProjection()
    ...
    #
    # return outDataset, outDataset.GetRasterBand(1)  #If you need to return, remenber to return  also the dataset because the band don`t live without dataset.
