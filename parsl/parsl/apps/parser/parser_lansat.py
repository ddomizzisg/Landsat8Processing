...
class ParserLansat(File):
    ...
    def __init__(self, content, filename, db, usrToken, catalog):
        ...
        super(ParserLansat, self).__init__(db)
    ...
    def parse(self):
        ...
        for l in self.lines:
            #print l
            if lat is not None and (l.startswith("ULLon") or l.startswith("URLon") or l.startswith("LLLon") or l.startswith("LRLon")):
                lon = self.cleanString(l)
                ...
                lat,lon = None,None
            elif l.startswith("ULLat") or l.startswith("URLat") or l.startswith("LLLat") or l.startswith("LRLat"):
                lat = self.cleanString(l)
            elif l.startswith("AcquisitionDate"):
                date = l.split(":")[1].strip()
            elif l.startswith("AcquisitionTime"):
                date += " " + l.split(":",1)[1].strip()
            elif l.startswith("Projection"):
