...
void Launcher::start(string mode)
{
	...
	else
	{
			...
			for (auto x : patterns)
			{
				up_command += " --scale " + x.second->getWorker()->getName()
								+ "=" + ::to_string(x.second->getNWorkers()) + " ";
			}
		...
		if (bb != NULL)
		{
			for (auto s : stg->getSourceStr())
			{
				...
				}else if (dirExists(s.c_str()) == 1)
				{
					...
					Logger("LAUNCHER: WARNING: " + s + "  doesn't exist or it isn't a directory", true);
				}
			}
